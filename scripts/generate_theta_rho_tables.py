...
def _load_tables(
    ...
    tables = {name: cells for name, cells in tables.items() if cells}
    if not tables:
        raise ValueError(
            "No compatible records were found in the supplied input glob(s)"
        )
    return tables
...
def main() -> None:
    ...
    thetas = _parse_csv_floats(args.thetas, auto_thetas)
    epsilons = _parse_csv_floats(args.epsilons, auto_epsilons)
    h_values = _parse_csv_floats(args.h_values, auto_h_values)
    if not thetas or not epsilons or not h_values:
        raise ValueError(
            "Could not discover a complete theta/epsilon/h grid from the input "
            "reports. Supply explicit values or check the input glob."
        )
