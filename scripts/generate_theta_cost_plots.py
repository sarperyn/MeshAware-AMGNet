...
def main() -> None:
    ...
    patterns = _parse_csv_text(args.patterns, auto_patterns)
    epsilons = _parse_csv_floats(
        args.epsilons, _available_epsilons(stats, patterns)
    )
    h_values = _parse_csv_floats(
        args.h_values, _available_h_values(stats, patterns, epsilons)
    )
    if not patterns or not epsilons or not h_values:
        raise ValueError(
            "Could not discover a complete pattern/epsilon/h grid from the "
            "input reports. Supply explicit values or check the input glob."
        )
    print(
        "Using "
